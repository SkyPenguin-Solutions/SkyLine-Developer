...
func RunFileCreationFromFile(args ...SLC_Object) SLC_Object {
	if x := CheckArguments("fpng.CreateImage", args, ExactArguments(1), ArgumentHasTypes(StringType)); x == nil {
		return &Boolean_Object{
...
func RunFileNewSettings(args ...SLC_Object) SLC_Object {
	if x := CheckArguments("fpng.PngSettingsNew", args, RangeArguments(0, 3), ArgumentHasTypes(StringType)); x == nil {
		out := args[0].SL_InspectObject()
		pw := args[1].SL_InspectObject()
		ph := args[2].SL_InspectObject()
		if out == "" {
			out = "SkyLine_Injected_File_Output_Image"
		}
		if pw == "" {
			pw = "600"
		}
...
func DEPEND_INVOKE_ARRAY_COPY(elems []SLC_Object) *Array {
	ARRAY_ELEMENTS := make([]SLC_Object, len(elems))
	copy(ARRAY_ELEMENTS, elems)
	return &Array{Elements: ARRAY_ELEMENTS}
}
...
func RegisterForensicsSubFunctions() {
	...
	RegisterBuiltin("ForensicsUtils.CreateImage", func(env *Environment_of_environment, args ...SLC_Object) SLC_Object {
